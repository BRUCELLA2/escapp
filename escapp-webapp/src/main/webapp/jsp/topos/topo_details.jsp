	...
	<div class="container">
		...
		<div class="panel panel-primary">
			...
			<div class="panel-body">
				<p>
					<b>Propriétaire : </b>
					<s:property value="ownerTopo.login" />
				</p>
				<p>
					<b>Département : </b>
					<s:property value="topo.department" />
				</p>
				<p>
					<b>Commune : </b>
					<s:property value="topo.municipality" />
				</p>
				<hr />
				<p>
				...
				<hr />
				<div class="panel-body">
					<s:a data-toggle="collapse" href="#collapseCommentTopo">Commentaires <span
							class="badge"><s:property value="nbCommentsTopo" /></span>
					</s:a>
				</div>
					...
					<div class="panel panel-default">
						<div id="collapseCommentTopo" class="panel-collapse collapse">
							<div class="panel-body">
								...
								<s:actionmessage />
								<s:iterator value="commentsTopoList" status="statusComTopoList"
									var="commentTopo">
									<div class="panel panel-info">
										<div class="panel-heading">
											<p>
												Commentaire
												<s:property value="#statusComTopoList.count" />
												-
												<s:property value="#commentTopo.value" />
											</p>
										</div>
										<div class="panel-body">
											<p>
												<s:property value="#commentTopo.key.text" />
											</p>
										</div>
									</div>
								</s:iterator>
								...
								<s:if test="#session.user">
									<form action="comment_topo">
										<div class="form-group">
											...
											<textarea class="form-control" rows="5" name="commentTopo"
												id="commentTopo"
												placeholder="Saisissez votre commentaire ici."></textarea>
											<div class="has-error">
												<span class="help-block"><s:fielderror
														fieldName="commentTopo" /></span>
											</div>
											<input type="hidden" name="topoId" id="topoId"
												value="<s:property value='topo.id'/>" />
										</div>
							...
							<s:if test="#session.user && #session.user.id != ownerTopo.id">
								<div class="col-xs-6 col-xs-offset-3">
									<form class="form-inline" method="post" action="book_topo">
										<div class="form-group">
											<label for="nbDays">Réserver le topo pour :</label> <input
												type="number" min="0" max="14" class="form-control"
												id="nbDays" placeholder="XX" name="nbDays"> <label
												for="nbDays"> jours</label> <input type="hidden" name="id"
												id="id" value="<s:property value='topo.id'/>" />
										</div>
							...
							<s:elseif test="#session.user.id == ownerTopo.id">
								<ul>
									...
									<li><s:a action="delete_topo">
										...
										Supprimer le topo</s:a></li>
								</ul>
							</s:elseif>
							<s:else>
								<p>Pour réserver ce topo, il est nécessaire de vous
									connecter.</p>
							</s:else>
						</s:if>
						<s:else>
							<s:if test="#session.user.id == borrowerTopo.id">
								...
								<p>
									Télécharger le topo :
									<s:a action="download">
										<s:property value="topo.pdfFileName" />
										<s:param name="fileName" value="topo.pdfFileName" />
									</s:a>
								</p>
							</s:if>
							<s:else>
								Ce topo est déjà réservé jusqu'au <fmt:parseDate
									value="${topo.endDateBorrow}" pattern="yyyy-MM-dd"
